...
class Web extends CI_Controller
{
   ...
   
 {
    ...
    $query = $this
                 ...
                 ->like('product_title',$user)
                 ->get('tbl_product');

     if($query->num_rows() > 0){
     foreach($query->result_array() as $row) {
        $new_row['value']=$row['product_title'];
        $new_row['label']=$row['product_title'];
    ...
    //  {z
    //   $temp_array = array();
    //   $temp_array['value'] = $row['product_title'];
    //   $temp_array['label'] = '<img src="uploads/'.$row['product_image'].'" width="70" />&nbsp;&nbsp;&nbsp;'.$row['product_title'].'';
    //   $output[] = $temp_array;
    //  }
    // }
    // else
    ...
    public function ajaxpros(){
        $searchTerm = $_GET['term'];

        // Get matched data from the database
        $query = "SELECT product_name,image,product_id FROM tbl_product WHERE (product_name LIKE '%".$searchTerm."%' )  ";
    ...
    public function searchs()
    {
        ...
        if (!empty($searchs)) {
            $data                    = array();
            $data['get_all_product'] = $this->web_model->get_all_searchs_product($searchs);
            $data['searchs']          = $searchs;

            if ($data['get_all_product']) {
               
                $this->load->view('web/pages/searchs', $data);
                //$this->load->view('web/pages/home', $data);
